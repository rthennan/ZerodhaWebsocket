...
def NFO_FULL():
    def log(txt):
    	...
    	if not path.exists(directory):
    		makedirs(directory)
    	logFile = path.join(directory,str(datetime.date.today())+'_DAS6_NFO_FULL.log')
    	logMsg = '\n'+str(datetime.datetime.now())+'    ' + txt
    	with open(logFile,'a') as f:
    		f.write(logMsg)   
    
    bnfoTokenList = pd.read_csv(path.join('lookup_tables','nfoTokenList.csv'))['instrument_token'].values.tolist()        
    tokenTable = np.load(path.join('lookup_tables','nfoTokenSymbol.npy'),allow_pickle=True).item()
    ...
    def replace(ticker):
        ...
        if 'instrument_token' in ticker:
            ...
            try:                
                c.execute("REPLACE INTO aws_das3full.`"+tokenTable[ticker['instrument_token']]+"` VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,\
