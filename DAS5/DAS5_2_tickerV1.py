...
def DAS5_2_Ticker():
    ...
    nseTokenList = pd.read_csv(path.join('lookup_tables','nseTokenList2.csv'))['instrument_token'].values.tolist()
    nseTokenTable = np.load(path.join('lookup_tables','nseTokenTable2.npy'),allow_pickle=True).item() 
    ...
    def log(txt):
    	...
    	if not path.exists(directory):
    		makedirs(directory)
    	logFile = path.join(directory,str(datetime.date.today())+'_DAS5_2_Ticker.log')
    ...
    try:
        ...
        msg = "DAS5_2 - Dump Failed. Exception : "   + str(e)   
        log(msg)
        mail(destinationEmailAddress,'DAS5 - Dump 2 Failed',msg)
        print("DAS5 - Dump 2  Failed. Exception : "+str(e))