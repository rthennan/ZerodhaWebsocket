...
def DAS5_4_Ticker():
    ...
    nseTokenList = pd.read_csv(path.join('lookup_tables','nseTokenList4.csv'))['instrument_token'].values.tolist()
    nseTokenTable = np.load(path.join('lookup_tables','nseTokenTable4.npy'),allow_pickle=True).item() 
    ...
    def log(txt):
    	...
    	if not path.exists(directory):
    		makedirs(directory)
    	logFile = path.join(directory,str(datetime.date.today())+'_DAS5_4_Ticker.log')
    ...
    try:
        ...
        msg = "DAS5_4 - Dump Failed. Exception : "   + str(e)   
        log(msg)
        mail(destinationEmailAddress,'DAS5 - Dump 4 Failed',msg)
        print("DAS5 - Dump 4  Failed. Exception : "+str(e))
