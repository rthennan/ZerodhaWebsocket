...
def log(txt):
    ...
    logFile = os.path.join(directory,str(datetime.date.today())+'_accessToken_DAS6.log')
...
for attempt in range(1,numbOfRetries+1):
	...
	try:
	    ...
	    c.execute("create table if not exists aws_tokens.das6 (time DATETIME UNIQUE,requestUrl varchar(255) ,reqToken varchar(255) ,accessToken varchar(255))")
	    shortsql = "INSERT into aws_tokens.das6 values (%s, %s, %s, %s)"
	    """Using DAS6 App"""
	    apikey = apiKey2
	    apisec = apisec2
	    """Using DAS App"""
	    ...
	    log("DAS6 - Access Token - Execution started")
	    print("DAS6 - Access Token - Execution started")

	    driver.get(url)
	    log("DAS6 - Entered Try Block")
	    "DAS6 - Access Token - Entered Try Block"
	    ...
	    print("Credentials Entered and submitted. Waiting for 2FA to load")
	    ...
	    log('Updated in DB')
	    print('Updated in DB')
	    msg = f'DAS6: Access Token Successful after {attempt} attempts'
	    log(msg)
	    print(msg)
	    mail(destinationEmailAddress,msg,'DAS6: Access Token succeeded at '+str(datetime.datetime.now())+'\n Proceeding to Instrument List file lookup')

	except Exception as e:
		msg = 'DAS7 - Access token Failed. Attempt No :'+str(attempt)+' Exception-> ' + str(e)+' Will retry after 30 seconds'
		log(msg)
		print(msg)
		time.sleep(30)
    ...
    msg = 'DAS6 - Access token Failed After '+str(numbOfRetries)+' attempts.'
    log(msg)
