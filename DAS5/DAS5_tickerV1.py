...
def DAS5_Ticker():
    ...
    nseTokenList = pd.read_csv(path.join('lookup_tables','nseTokenList.csv'))['instrument_token'].values.tolist()
    nseTokenTable = np.load(path.join('lookup_tables','nseTokenTable.npy'),allow_pickle=True).item() 
    indexTokenList = pd.read_csv(path.join('lookup_tables','indexTokenList.csv'))['instrument_token'].values.tolist()
    indTokenTable = np.load(path.join('lookup_tables','indextable.npy'),allow_pickle=True).item()   
    tokenList = nseTokenList + indexTokenList
    ...
    def log(txt):
    	...
    	if not path.exists(directory):
    		makedirs(directory)
    	logFile = path.join(directory,str(datetime.date.today())+'_DAS5_Ticker.log')
    ...
    def replace(ticker):
        ...
        if 'instrument_token' in ticker:
            ...
            if ticker['instrument_token'] in indexTokenList:
            #Inserting Index values
            
                try:
                    c.execute("REPLACE INTO aws_das2daily.`"+indTokenTable[ticker['instrument_token']]+"` VALUES (%s,%s)",[ticker['timestamp'],ticker['last_price']])
    ...
    try:
        ...
        ce = conne.cursor()
        ce.execute('select accessToken from das5 order by time desc limit 1')
        ...
        msg = "DAS5 - NSE Dump Failed. Exception : "   + str(e)   
        log(msg)
        mail(destinationEmailAddress,'DAS5 - NSE Dump Failed',msg)
        print("DAS5 - NSE Dump Failed. Exception : "+str(e))